...
const Navbar = () => {
  const {productData, cartData, setFilteredProducts, isCartActive, setIsCartActive} = useContext(ProductContext)
  
  const intervalRef = useRef(null);
  const searchData = ["paneer", "rice", "chocolate", "milk", "bread", "butter"];
  const [currentIndex, setCurrentIndex] = useState(0);
  
  // TOTAL QUANTITY & ITEMS CALCULATION
  const totalQty = cartData.reduce((total, item) => total + (item.qty || 0), 0);
  const totalItems = cartData.length;

  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % searchData.length);
    }, 1500);
  ...
  return (
    <nav className='h-21.5 w-full border-b border-zinc-200 flex items-center justify-between px-2.5'>
      {/* LOGO */}
      <div className="logo w-44.5 h-full border-r border-zinc-200 flex items-center justify-center">
      ...
      {/* LOCATION */}
      <div className="location w-80 h-full flex flex-col items-center text-start justify-center">
        <h1 className='text-xl font-bold'>Delivery in 8 minutes</h1>
        <select name="" id="">
          <option value="">Nai Basti Deoli</option>
        </select>
      </div>
      ...
      <div className="search relative w-172 h-auto bg-slate-50 border border-zinc-200 rounded-xl pl-3 flex items-center justify-center">
        ...
        <Link to="/sr" className='w-full h-12 pl-2 outline-none text-zinc-600'>
          <input 
            ...
            type="text" 
            placeholder={`Search "${searchData[currentIndex]}"`}
          />
        </Link>
      </div>
      ...
      <div className="loginCart w-80.75 h-full flex items-center justify-center">
        <div className="login w-1/2 h-full text-lg flex items-center justify-center">
          <Link>Login</Link>
        </div>
        
        <div onClick={()=> setIsCartActive(true)} className="cart w-1/2 h-full flex items-center justify-center cursor-pointer relative group">
          
      
          
          <div className="cartButton w-28 h-13 flex items-center justify-center gap-2 bg-green-600 rounded-lg hover:bg-green-700 transition-all duration-200 shadow-md group-hover:shadow-lg">
            <MdOutlineShoppingCart className='text-2xl text-white' />
            
            {cartData.length > 0 ? (
              <div className="text-right leading-tight">
                <h1 className='font-bold text-white text-sm'>{totalQty} Items</h1>
                <p className='text-xs text-gray-200'>({totalItems} types)</p>
              </div>
            ) : (
      ...
      {/*  CART OVERLAY */}
      {isCartActive ? (
        <div className='w-full h-full fixed z-50 top-0 left-0 '>
          <Cart/>
        </div>
      ) : null}
    </nav>
